...
\end{thm}
\begin{bukti}
    Diperhatikan bahwa $W$ adalah himpunan kompak, ini berarti ada subbarisan $\{x_{n_k}\}$ dari $\{x_n\}$ yang konvergen kuat ke $x\in W$ atau $\lim_{k\to\infty} d(x_{n_k},x)=0$. Akibatnya $\{x_{n_k}\}$ juga konvergen$-\Delta$ ke $x\in W$.  Dengan menggunakan fakta bahwa $W$ himpunan kompak dan $Fix(T)\neq\emptyset$, maka berdasarkan Lema \ref{Lema:xntxn0}, diperoleh
    \begin{align*}
...
\begin{defn}\label{defn:lajukonv}\cite{Berinde2015}
    Diberikan ruang $CAT_p(0)$ $(X,d,G)$. $\{x_n\}$ adalah barisan yang konvergen ke $x\in X$. Jika terdapat $r\in [1,2]$ yang memenuhi 
    \begin{align*}
        \lim_{n\to\infty} \dfrac{d(x_{n+1},x^*)}{\qty[d(x_n,x^*)]^r}=\lambda <+\infty,
    \end{align*}
    maka $r$ disebut sebagai laju konvergensi dari $\{x_n\}$ dengan $\lambda$ sebagai galat asimtotik. Jika $r=1$, laju konvergensinya linear, jika $r=2$, laju konvergensinya kuadratik, dan jika $1<r<2$, laju konvergensinya superlinear.
\end{defn}
\begin{thm}\label{thm:laju}
Diberikan $(X,d,G)$ adalah ruang $CAT _p(0)$ yang lengkap dan $W$ adalah himpunan bagian tak kosong dari $X$ yang tertutup, konveks, dan kompak. Jika $T:W\to W$ adalah pemetaan $(\alpha,\beta,\gamma)$-nonekspansif dengan $\alpha+\gamma<1$, $Fix(T)\neq \emptyset$, serta $\{x_n\}$ adalah barisan yang dikonstruksi melalui skema iterasi Sabri \eqref{eq:sabricat} dengan $\{a_n\},\{c_n\}\subset (0,1)$, maka $\{x_n\}$ memiliki laju konvergensi linear dengan galat asimtotik $\lambda\leq \frac{\alpha^3}{(1-\gamma)^3}$.
\end{thm}
\begin{bukti}
    ...
    \end{align*}
    Hal ini berarti $\{x_n\}$ memiliki laju konvergensi linear dengan galat asimtotik $\lambda\leq \frac{\alpha^3}{(1-\gamma)^3}$.
\end{bukti}
% \begin{bukti}
%     \begin{align*}
%         d(q_n,x^*) &=d\qty(T\qty((1-c_n)x_n\oplus c_n Tx_n),Tx^*)\\
