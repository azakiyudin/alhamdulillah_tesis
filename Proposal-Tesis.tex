...
\TanggalDisetujui{20 Januari 2026} %  Isi dengan tanggal seminar
\allowdisplaybreaks
...
$\qty(d(x,y))^p$ &:& Perkalian dari $d(x,y)$ dengan $d(x,y)$ sebanyak $p$ kali\\
$:=$ &:& Didefinisikan\\
...
$\displaystyle \lim_{n\rightarrow \infty} x_n$ &:& Limit barisan $\{x_n\}_{n=1}^{\infty}$\\
...
% \fancyhead[RE,LO]{Chapter \thechapter} % Even pages right, odd pages left
\fancyfoot[LE,RO]{\thepage} % Even pages left, odd pages right
\renewcommand{\headrulewidth}{0pt}

% --- Chapter Page Style Overwrite ---
% This forces the first page of a chapter to be center-bottom
\fancypagestyle{plain}{%
  \fancyhf{}%
  \fancyfoot[R]{\thepage}%
  \renewcommand{\headrulewidth}{0pt}%
